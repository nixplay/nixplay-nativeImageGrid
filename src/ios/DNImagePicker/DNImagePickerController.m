...
- (id)init:(bool)allow_v withAssets: (NSArray*)preSelectedAssets {
    if (self = [super init]) {
        _selectedAssets = [preSelectedAssets copy];
    }
    return self;
}
                                     ...
                                     usingBlock:^(ALAssetsGroup *assetsGroup, BOOL *stop)
         {
             ...
             if ([assetsGroupID isEqualToString:propwetyID]) {
                 *stop = YES;
                 ...
                 for(NSString *assetString in _selectedAssets) {
                     [imageFlowController.assetsLibrary assetForURL:[NSURL URLWithString:assetString] resultBlock:^(ALAsset *asset) {
                         [imageFlowController addPreSelectedAssetsObject: asset];
                     } failureBlock:^(NSError *error) {
                         NSLog(@"Error when selecting resources");
                     }
